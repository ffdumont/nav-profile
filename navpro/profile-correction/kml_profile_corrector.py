...
{chr(10).join(new_placemarks)}
    ...
    def generate_airspace_kml(self, input_file: str, output_file: str):
        ...
        try:
            ...
            # Add the parent directories to the path to import airspace checker modules
            # Now we're inside navpro/profile-correction, so parent is navpro, grandparent is project root
            navpro_dir = os.path.dirname(os.path.abspath(__file__))  # navpro/profile-correction
            navpro_parent = os.path.dirname(navpro_dir)  # navpro
            project_root = os.path.dirname(navpro_parent)  # project root
            
            # Add paths for imports
            sys.path.insert(0, project_root)
            sys.path.insert(0, navpro_parent)
